...
class StockPriceService:
    ...
    def fetch_prices_for_articles(self, limit: int = 50, days_around: int = 2):
        """
        ...
        try:
            with connection.cursor() as cursor:
                # Find articles with tickers that don't have snapshots yet
                cursor.execute(
                    """SELECT DISTINCT ri.id, ri.title, ri.stock_tickers, ri.published_at
                       ...
                       WHERE ri.stock_tickers IS NOT NULL
                         ...
                         AND ri.id NOT IN (
                             SELECT DISTINCT article_id FROM article_stock_snapshots
                         )
